 ...
 *
 * Mirrors the real /report page: month selector → 共同出費 table with
 * spent/budget/remaining columns and a usage bar per row → 収支集計
 * card at the bottom.
 *
 * Animation is one-shot (replays each time the slide remounts) — bars
 * grow once, then hold at their final width. No infinite loop, so no
 * flicker at the loop boundary.
 *
 * Color rule for the usage bar:
 *   - within budget       → primary (blue)
 *   - over budget (>100%) → destructive (red)
 * That keeps the visual to a single accent + an alarm color, instead
 * of a full red/green stoplight that competes for attention.
 */
  ...
  return (
    ...
    >
      ...
      <g className="rep-summary">
        ...
        <text x="30" y="210" fontSize="11" fontFamily="system-ui" fontWeight="700" fill="hsl(var(--foreground))">¥520,000</text>
        ...
        <text x="230" y="210" fontSize="11" fontFamily="system-ui" fontWeight="700" fill="hsl(var(--foreground))">+¥133,800</text>
      </g>

      <style>{`
        /* One-shot: animations play once and stay (forwards); the SVG remounts
           when the slide is shown, so they replay automatically on re-watch. */
        .rep-illustration .rep-header { opacity: 0; transform: translateY(2px); animation: rep-fade 500ms ease-out 0ms forwards; }
        .rep-illustration .rep-table  { opacity: 0; transform: translateY(2px); animation: rep-fade 500ms ease-out 200ms forwards; }
        .rep-illustration .rep-row    { opacity: 0; transform: translateX(-4px); }
        .rep-illustration .rep-row-0  { animation: rep-row 500ms ease-out 500ms forwards; }
        .rep-illustration .rep-row-1  { animation: rep-row 500ms ease-out 750ms forwards; }
        .rep-illustration .rep-row-2  { animation: rep-row 500ms ease-out 1000ms forwards; }
        .rep-illustration .rep-row-3  { animation: rep-row 500ms ease-out 1250ms forwards; }
        .rep-illustration .rep-bar    { transform-origin: left center; transform: scaleX(0); }
        .rep-illustration .rep-row-0 .rep-bar { animation: rep-bar 700ms ease-out 700ms forwards; }
        .rep-illustration .rep-row-1 .rep-bar { animation: rep-bar 700ms ease-out 950ms forwards; }
        .rep-illustration .rep-row-2 .rep-bar { animation: rep-bar 700ms ease-out 1200ms forwards; }
        .rep-illustration .rep-row-3 .rep-bar { animation: rep-bar 700ms ease-out 1450ms forwards; }
        .rep-illustration .rep-summary { opacity: 0; transform: translateY(6px); animation: rep-summary 600ms ease-out 2200ms forwards; }

        @keyframes rep-fade   { to { opacity: 1; transform: translateY(0); } }
        @keyframes rep-row    { to { opacity: 1; transform: translateX(0); } }
        @keyframes rep-bar    { to { transform: scaleX(1); } }
        @keyframes rep-summary{ to { opacity: 1; transform: translateY(0); } }
