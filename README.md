...
## 🔐 プライバシーと「自分でデプロイすべき」理由

**短くいうと**: 誰かが運営しているインスタンス（例: 友人がデプロイした URL）にアカウントを作る場合、その運営者は技術的にあなたの家計データにアクセスできます。これはホスト型 SaaS の構造的な特性で、Notion / Slack / Asana など多くのサービスで同じです。

| 守られている部分 | 守られていない部分 |
| --- | --- |
| ✅ 別の世帯ユーザーから（RLS で完全分離） | ❌ そのインスタンスを運営している人から |
| ✅ 外部からの不正アクセス（HTTPS + Supabase 認証） | ❌ Supabase の Dashboard / service_role キーを持つ人 |

**プライバシーを最大化したい人へのおすすめ — 自分でデプロイ**:
1. このリポジトリを Fork または Clone
2. 自分の Vercel + Supabase アカウントで [セットアップ手順](#セットアップ手順) を実行（30 分程度）
3. 自分専用の URL ができ、データベースもあなただけが管理する
4. 運営者 = あなた自身、なので技術的にも誰からも見られない

> 「家族のためにホストして共有する」場合も、家族が運営者（あなた）を信頼している前提が必要です。OSS なので、家族にも「自分でデプロイしたい？」と聞くのが理想。
